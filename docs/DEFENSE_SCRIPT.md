# Defense Script — Invoice Approval Workflow

## 30-second opening

“This is not a simulator of green workflow nodes. It is an invoice-approval application. A user submits one invoice PDF and metadata. The system validates it, pauses for a real human decision, resumes the same persisted run, archives an approved document or records another controlled outcome, creates an internal notification, and preserves the audit trace. I implemented that same business process with orchestration and choreography so their control-flow trade-off can be compared without changing the business rules.”

## Five-minute demonstration

### 1. Establish the product value — 40 seconds

Open `/ui`. Point to the invoice form and say what the user receives: invoice identity, current business state, required next action, final result, execution mode, and optional audit evidence.
...
2. Submit the prepared valid PDF and metadata.
3. Show `PENDING_APPROVAL` and “Review and decide this invoice.”
4. Explain that the HTTP request finished and the cursor/work item are persisted.
5. Approve the invoice.
6. Show `ARCHIVED`, “Approved and archived,” and “No action required.”
...
Explain: “Choreography changes who triggers the next committed step. It does not own another copy of invoice, retry, or routing policy. The handler is scoped to one active run and is removed while waiting.”
...
It does not process a bank payment. The bounded product is invoice approval. Outcomes come from metadata/PDF validation, the human approve/reject decision, archive behavior, and deterministic test faults. A real payment adapter is outside scope.
...
“The project’s significance is controlled, explainable workflow behavior around failure and time: invalid input cannot reach approval, retries are bounded, a human decision can arrive after restart, duplicate decisions cannot advance twice, and both control strategies produce the same invoice outcome.”
