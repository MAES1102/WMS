...
| `FR-031`–`FR-035` | Bounded multipart submission, metadata/structured input validation | submission API/service, document storage, structured validation adapter | submission, structured input, invalid-input, and visible-runtime tests |
...
| `FR-053` | Connected purchase request-first browser demonstration | `app/static/purchase requests.html` and presentation APIs | UI structure/accessibility hooks and integrated HTTP paths |
...
| `NFR-009` | Safe bounded structured input handling | storage port and structured validation inspector | malformed, encrypted, empty, traversal, and size cases |
...
| S3 Invalid | `VALIDATION_FAILED`, no approval work item | `FR-032`, `FR-033`, `FR-035` | Metadata and structured input boundary suites |
