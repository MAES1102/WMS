# Requirements — Invoice Approval Workflow

| Field | Value |
|---|---|
| Document | Requirements — Invoice Approval Workflow |
...
CR-002 changed the demonstration from an abstract workflow runner into the bounded invoice-approval product described here. This is the current specification. Requirement identifiers retain their historical numbering so code, tests, architecture decisions, and report evidence remain traceable.
...
| Demonstration scenario | Explicit deterministic archive behavior selected to show the normal, retry, or exhausted-failure path; it is not random and does not replace the invoice or approval decision. |
...
| Review invoice | `HUMAN_APPROVAL` | Not applicable; human tasks are never automatically retried |
...
The automated suite executes all five business paths in both modes, compares normalized results, verifies deterministic retry behavior, exercises the constructor boundary, and checks persistence, restart, isolation, PDF safety, trace order, dependencies, deployment inventory, and UI structure.
...
Review result: the specification contains 49 functional requirements, 11 non-functional requirements, and 14 constraints. Every active FR and NFR has forward evidence in the synchronized traceability record. The project repository contains the implementation and automated verification; final academic acceptance remains the professor's decision.
