# Requirements — Event-Driven Workflow Management System
...
| Approval work item | Persistent pending request associated with exactly one purchase request, run, and human-approval task. |
...
| Demonstration scenario | Explicit deterministic authorization behavior selected to show the normal, retry, or exhausted-failure path; it is not random and does not replace the purchase request or approval decision. |
...
| Request | `SUBMITTED`, `VALIDATION_FAILED`, `PENDING_APPROVAL`, `APPROVED`, `REJECTED`, `AUTHORIZED`, `NEEDS_MANUAL_ACTION` |
...
| `FR-026` | Modified | Trace adds purchase request, waiting/resume, approval, business-state, and notification observations. |
...
| `FR-030` | Superseded by `FR-053` | The UI must lead with purchase request work and outcome rather than scenario selectors and abstract task indicators. |
| `NFR-001`–`NFR-008` | Retained with the v3 wording below | Their scope is updated to the bounded purchase request scenarios and waiting state. |
...
### 5.3 Request submission and validation
...
- Review `SUCCESS` → Authorization;
- Review `FAILURE` → Notify;
- Authorization `SUCCESS` → Notify;
- Authorization `FAILURE` → Notify;
