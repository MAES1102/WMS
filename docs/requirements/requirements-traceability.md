...
|---|---|---|---|---|---|---|
| `FR-001` | CR-001 conditional graph and explicit nodes/transitions | PBI-E06 | EV-018, EV-019 | Definition inspection and graph test | `A4`: ADR-002, ADR-003; domain model, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-002` | Minimal rule 1: exactly one designated start | PBI-E06 | EV-019; CR-001 transition impact | Invalid-definition test | `A4`: ADR-003; domain model, routing/retry activity, run state; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
...
| `FR-005` | Minimal rule 3; arbitrary-cycle exclusion | PBI-E06, PBI-E09 | EV-022, RK-005 | Self-edge and directed-cycle rejection tests | `A4`: ADR-003; domain model, routing/retry activity; `T6` | Approved baseline requirement; archive cycle artifacts remain comparison evidence; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-006` | CR-001 complete definition validation before execution, including attempt bounds | PBI-E06 | CR-001 impact analysis, RK-005 | Validation-path inspection and rejection-with-no-execution-state tests | `A4`: ADR-003, ADR-006; system context/use cases, both sequence views, routing/retry activity, run state; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-007` | CR-001 `SUCCESS`/`FAILURE`/`ALWAYS` conditions | PBI-E06 | EV-019 | Condition-domain validation test | `A4`: ADR-003; domain model, routing/retry activity; `T6` | Approved baseline requirement; archive source presence is not runtime proof; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-008` | Minimal rule 4: outcome-specific precedence | PBI-E06 | EV-019 | Resolver and trace-selection test | `A4`: ADR-003; both sequence views, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-009` | Minimal rule 5: `ALWAYS` fallback | PBI-E06 | EV-019 | Resolver fallback test | `A4`: ADR-003; both sequence views, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-010` | Minimal rule 6: reject equal-precedence ambiguity | PBI-E06 | CR-001 transition impact | Duplicate-eligibility rejection test | `A4`: ADR-003; routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-011` | Minimal rule 7: successful terminal | PBI-E06 | CR-001 transition impact | Terminal-semantics test | `A4`: ADR-003; both sequence views, routing/retry activity, run state; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-012` | Minimal rule 7: unsuccessful terminal | PBI-E06 | CR-001 transition impact | Terminal-semantics test | `A4`: ADR-003; both sequence views, routing/retry activity, run state; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-013` | CR-001 one shared workflow model | PBI-E05, PBI-E06 | EV-019, EV-020, RK-004 | Design review and cross-mode definition identity check | `A4`: ADR-001, ADR-002; component view, domain model; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-014` | CR-001 shared transition-resolution rule set | PBI-E06 | EV-019, RK-004 | Paired resolver and trace comparison | `A4`: ADR-003; component view, both sequence views, routing/retry activity; `T6` | Approved baseline requirement; the accepted shared resolver design is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-015` | CR-001 centralized orchestration | PBI-E07 | EV-018, EV-019 | Orchestration acceptance cases | `A4`: ADR-004; component view, orchestration sequence; `T6` | Approved baseline requirement; baseline source presence is not current runtime proof; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation has not started |
| `FR-016` | CR-001 in-memory EventBus choreography | PBI-E08 | EV-018, EV-019, EV-023, EV-024 | Choreography acceptance cases without broker | `A4`: ADR-005; component view, choreography sequence, deployment view; `T6` | Approved baseline requirement; Kafka runtime remains `UNVERIFIED` and excluded; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation has not started |
| `FR-017` | Minimal rule 11; semantic parity | PBI-E07, PBI-E08, PBI-E10 | RK-004 | Paired normalized-trace comparison | `A4`: ADR-003, ADR-004, ADR-005, ADR-007; both sequence views, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; the six cases have not been executed |
| `FR-018` | CR-001 run-specific execution state | PBI-E05 | EV-020, EV-021, RK-003 | Run-state association inspection | `A4`: ADR-002, ADR-006; domain model, component view, run state; `T6` | Approved baseline requirement; archive artifacts remain comparison evidence; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation has not started |
| `FR-019` | CR-001 sequential run isolation | PBI-E05 | EV-020, RK-003 | Repeated-run isolation test | `A4`: ADR-002, ADR-004, ADR-005, ADR-006; domain model, component view, run state; `T6` | Approved baseline requirement; archive test definitions are not pass evidence; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; verification has not started |
| `FR-020` | CR-001 concurrent run isolation | PBI-E05 | EV-020, RK-003 | Controlled-concurrency isolation test | `A4`: ADR-002, ADR-004, ADR-005, ADR-006; component view, choreography sequence, run state; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-021` | Minimal rule 8: positive maximum-total-attempt bound for every task | PBI-E09 | RK-005 | Attempt-bound validation and boundary tests | `A4`: ADR-003; domain model, both sequence views, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-022` | Minimal rule 8: retry if and only if the bound remains | PBI-E09 | EV-022, RK-005 | Two-sided boundary tests proving the required retry below the bound and prohibited retry at the bound for bounds one and two | `A4`: ADR-003; both sequence views, routing/retry activity; `T6` | Approved baseline retry rule; the archive loop counter is not accepted behavior; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation has not started |
| `FR-023` | Minimal rule 9: persistent attempt state | PBI-E05, PBI-E09 | EV-020, RK-005 | Attempt-state persistence inspection | `A4`: ADR-002, ADR-003, ADR-006; domain model, both sequence views, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-024` | Minimal rule 9: retry without graph edge | PBI-E09 | EV-022, RK-005 | Retry trace inspection | `A4`: ADR-003; both sequence views, routing/retry activity; `T6` | Approved baseline retry rule; cycle-capable prototype behavior is excluded; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation has not started |
| `FR-025` | Minimal rule 10: resolution after exhaustion | PBI-E09 | EV-019, EV-022, RK-005 | Exhaustion resolver test | `A4`: ADR-003; both sequence views, routing/retry activity; `T6` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
| `FR-026` | CR-001 persistent execution trace | PBI-E05, PBI-E10 | CR-001 impact analysis, RK-004, RK-011 | Trace-content and restart test | `A4`: ADR-002, ADR-004, ADR-005, ADR-006; component view, domain model, both sequence views, run state; `T6`; `D8` | Approved baseline requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; no trace execution evidence exists, and implementation has not started |
...
| `NFR-005` | CR-001 system-wide completeness of pre-execution validation | PBI-E06, PBI-E10 | RK-005 | Validation-path inspection plus parameterized invalid, valid-boundary, and combined-violation cases | `A4`: ADR-003; routing/retry activity, run state; `T6` | Approved baseline quality requirement; accepted checkpoint-4 design mapping is approved; Git durability requires a separate authorized and verified action; implementation and verification have not started |
...
| One shared workflow model | `FR-013` | PBI-E05, PBI-E06 | EV-019, EV-020; accepted ADR-001/ADR-002 and component/domain views are approved checkpoint-4 design evidence; Git durability requires a separate authorized and verified action |
...
| UML or ADR decisions | `CON-010` | Requirements content inspection | Requirements v2 remains design-neutral; checkpoint-4 ADR/UML artifacts are accepted design evidence, with Git durability requiring a separate authorized and verified action |
...
- Checkpoint-4 architecture/ADR/UML passed substantive project review and is the approved design baseline; Git durability requires a separately authorized and verified commit/ref action, while implementation, executable tests, deployment, and release remain not started.
