# Invoice Approval Workflow — Architecture Overview
...
| Mutable `Task.status`/timestamps compatibility fields | Removed | Run state now belongs only to invoice-run persistence. |
...
| Existing runtime routes/engines | Removed after vertical cutover | One invoice application now owns the runtime boundary. |
