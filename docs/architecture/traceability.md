...
| `FR-031`–`FR-035` | Submission, bounded metadata/structured input validation | ADR-001, ADR-002, ADR-008 | context, component, domain, deployment |
| `FR-036`–`FR-042` | Persistent human waiting, decision, same-run resume | ADR-002, ADR-004–ADR-006 | domain, run state, both sequences |
| `FR-043`–`FR-047` | Authorization, deterministic faults, notification, projection | ADR-002, ADR-003, ADR-006–ADR-008 | component, domain, routing activity |
| `FR-053`, `NFR-011` | Request-first browser experience | ADR-001, ADR-009 | context and component |
...
| ADR-002 | Draft/revision/run/purchase request ownership | model, constructor, run-state tests |
...
| ADR-008 | structured input/document boundary | storage and malformed-structured input tests |
...
| Source | Purpose |
|---|---|
| [system-context-use-cases.puml](./uml/system-context-use-cases.puml) | Actors and useful purchase request goals |
| [component-view.puml](./uml/component-view.puml) | Module responsibilities and dependencies |
| [domain-model.puml](./uml/domain-model.puml) | Definition, run, purchase request, approval, and trace ownership |
...
Every source has a matching SVG and structured input render under `docs/architecture/uml/rendered*`.
