# Invoice-Workflow Architecture Decisions v3
...
| Previous decision set | ADR-001–ADR-007 checkpoint-4 baseline |
| Implementation | `PBI-E17 FOUNDATION ACCEPTED — project level` |

The existing ADR identifiers retain their concerns and record their v3 amendments. ADR-008 and ADR-009 are new. Implementation evidence is recorded separately in EV-032–EV-034; no instructor approval is claimed.

## ADR-001 — Retain the single-deployable layered modular monolith

**Status:** `ACCEPTED — architecture v3 baseline`

**Context.** Requirements v3 adds business data, human waiting, controlled files, and constructor CRUD but still excludes distributed execution and external business dependencies.

**Decision.** Retain one FastAPI process with presentation, application, domain, and infrastructure layers. Add cohesive invoice, approval, definition/revision, execution, and query application services. Dependencies point toward shared domain policy. Database and controlled document storage remain infrastructure boundaries.

**Rejected alternatives.** Microservices/broker topology adds unsupported operational complexity; embedding all logic in routes repeats the coupling that made the prototype difficult to evolve; a second invoice application would split one product into two systems.
...
## ADR-002 — Separate drafts, immutable revisions, run cursor, and invoice state

**Status:** `ACCEPTED — architecture v3 baseline`

**Context.** C5A separates definition and run concepts but lacks immutable revisions, invoice ownership, human waiting, and a recoverable execution cursor. Old task-definition status/timestamps remain compatibility debt.
...
- invoice-owned document metadata, business state, approval work item, archive record, and notification.

Every run references one invoice and one immutable revision. New code never mutates definition-task runtime fields. Earlier runs keep their revision after later draft edits.

**Rejected alternatives.** Mutating an active graph makes old traces uninterpretable; copying state onto task definitions breaks isolation; treating invoice status as the run status loses the difference between business and execution outcomes.
...
**Context.** A single `SUCCESS`/`FAILURE` outcome cannot determine whether a failure should be retried. Invalid invoices and human rejection are business outcomes; transient archive/notification errors may be retried.
...
**Context.** Real invoice inputs and decisions create understandable behavior, while repeatable retry/parity verification still needs controlled technical failures.

**Decision.** Normal product execution uses PDF/metadata, explicit approval decisions, and bounded local task behavior. Test/defense configuration may select a deterministic fault schedule keyed by stable run/task configuration. It wraps the executor port and is never selected by task display name. Both modes use the same schedule. Deployment remains one FastAPI application with database and controlled document storage and no external business service or broker.
...
**Consequences.** Product value and verification mechanism are clearly separated. Reports must disclose the PDF library and behavioral references without presenting them as custom implementation.

**Traceability.** `FR-017`, `FR-045`, section 9, `NFR-001`, `NFR-002`, `NFR-008`, CON-005, CON-008, CON-014; RK-007, RK-019, RK-021.

## ADR-008 — Use a bounded PDF/document adapter and generated storage identities
...
**Decision.** The request boundary enforces a streaming size limit. A document-storage port generates the storage identity; original filename is metadata only. A selected maintained PDF library behind an adapter checks openability, encryption/readability, and page count. Domain validation owns metadata rules. Archive records reference the generated document identity. Rejected temporary content follows an explicit cleanup path selected during implementation design.

**Rejected alternatives.** Original filenames as paths enable collisions/traversal; reading unbounded files into memory violates the boundary; OCR/AI expands scope and external dependencies; embedding PDF APIs in routes couples infrastructure to use cases.
...
- ADR-008 owns PDF/storage integration.
- ADR-009 bounds constructor extensibility.

No ADR authorizes implementation before the synchronized UML and traceability set passes review.
