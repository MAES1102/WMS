...
|---|---|---:|---:|---|---|
| R-01 | Evaluator sees only technical nodes and cannot identify product value | Medium | High | Invoice-first UI, five-minute script, business states, real approval decision | Low |
...
| R-06 | Two runs contaminate each other's data | Low | High | Run/invoice keys, unique constraints, version checks, threaded isolation tests | Low |
| R-07 | Uploaded file escapes storage or invalid PDF reaches approval | Medium | High | Generated identity, bounded size, pypdf adapter, traversal and malformed-file tests | Low |
...
| R-11 | Local database schema is stale after pulling a new version | Medium | Medium | Runtime DB is ignored and disposable for demonstration; startup instructions use a clean `invoice.db` | Low |
